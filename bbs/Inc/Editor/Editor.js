...
KE.init = function(config) {
    ...
    config.skinType = config.skinType || 'tinymce';
    config.cssPath = config.cssPath || '';
    config.skinsPath = 'Inc/Editor/images/';
    config.pluginsPath = 'Inc/Editor/images/';
...
KE.plugin['emoticons'] = {
    ...
    click : function(id) {
        ...
        for (var i = 0; i < emoticonTable.length; i++) {
            ...
            for (var j = 0; j < emoticonTable[i].length; j++) {
                ...
                var img = KE.$$('img');
                img.src = KE.g[id].pluginsPath + 'face/' + emoticonTable[i][j];
    ...
    exec : function(id, value) {
        KE.util.select(id);
        var html = '<img src="' + KE.g[id].pluginsPath + 'face/' + value + '" border="0">';
        KE.util.insertHtml(id, html);
        KE.layout.hide(id);
        KE.util.focus(id);
    }
...
document.form1.content.value=KE.util.getData('content');
